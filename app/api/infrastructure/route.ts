...
export async function GET() {
  try {
    const infrastructureRef = db.collection('infrastructure')
    const snapshot = await infrastructureRef.get()
...
export async function POST(req: NextRequest) {
  try {
    ...
    const infrastructureRef = db.collection('infrastructure')
    const docRef = await infrastructureRef.add(infrastructureData)
